...
@endif
                ...
                @switch(@$field['type'])
                    @case('checkbox')
                        <label class="cursor-pointer label mt-4">
                            ...
                            <span class="label-text text-left w-full ml-2">
                                {!! $field['label'] !!} <span class="text-red-600 inline-block"
                                    title="Obrigatório">{{ $required }}</span><br />
                                <span class="text-gray-400">{!! @$field['placeholder'] !!}</span>
                                @if (isset($field['help']))
                                    <i class="text-gray-400 bi bi-info-circle" title="{{ $field['help'] }}"></i>
                                @endif
                            </span>
                        </label>
                    @break
                    ...
                    @case('select')
                        <label class="label">
                            <span class="label-text">{!! $field['label'] !!} <span class="text-red-600 inline-block"
                                    title="Obrigatório">{{ $required }}</span></span>
                            @if (isset($field['help']))
                                <i class="text-gray-400 bi bi-info-circle" title="{{ $field['help'] }}"></i>
                            @endif
                        ...
                        <select wire:model="{{ $key }}"
                            class="select select-bordered w-full @error($key) select-error @enderror">
                    ...
                    @case('radio')
                        <span class="label-text">
                            {!! $field['label'] !!} <span class="text-red-600 inline-block"
                                title="Obrigatório">{{ $required }}</span>
                            @if (isset($field['help']))
                                <i class="text-gray-400 bi bi-info-circle" title="{{ $field['help'] }}"></i>
                            @endif
                        </span>
                        <div class="flex flex-row mt-2 mb-2 flex-wrap">
                            @foreach ($field['options'] as $info)
                                <label class="cursor-pointer mr-5 mb-2">
                                    <div class="d-flex align-items-center">
                                        <div class="d-flex">
                                            <input wire:model="{{ $key }}" type="radio" checked="checked"
                                                class="radio radio-primary" value="{{ $info['id'] }}">
                                        </div>
                                        <div class="ml-1">
                    ...
                    @case('date')
                        ...
                        <input wire:model="{{ $key }}" type="date" name="{{ $key }}"
                            placeholder="{{ @$field['placeholder'] }}"
                            class="input input-bordered @error($key) input-error @enderror max-w-md" {{ @$field['attr'] }} />
                    @break
                    ...
                    @case('poll')
                        <div class="flex w-full text-wrap">
                            <div class="w-1/2">
                                ...
                                <textarea wire:model="{{ $key }}" name="{{ $key }}" placeholder="{{ @$field['placeholder'] }}"
                                    class="textarea textarea-bordered h-40 w-full @error($key) textarea-error @enderror"></textarea>
                            </div>
                            <div class="w-1/2">
                                <label class="label">
                                    <span class="label-text">Visualização</span>
                                </label>
                                <div x-data="{ content: null }" x-init="{ content: @entangle('data.poll') }"
                                    class="w-full p-2 bg-gray-200 h-40 overflow-scroll">
                    ...
                    @case('textarea')
                        ...
                        <textarea wire:model="{{ $key }}" name="{{ $key }}" placeholder="{{ @$field['placeholder'] }}"
                            class="textarea textarea-bordered h-48 mb-2 @error($key) textarea-error @enderror"></textarea>
                    @break
        ...
        @if (isset($data['id']) || $editing)
            <button wire:click="update()" wire:loading.attr="disabled" wire:loading.class="loading"
                class="btn btn-primary float-right">Salvar</button>
            @if (!isset($edit))
                <button wire:click="cancel()" class="btn float-right mr-6">Cancelar</button>
            @endif
        @else
            <button wire:click="store()" wire:loading.attr="disabled" wire:loading.class="loading"
                class="btn btn-wide btn-primary">Enviar</button>
        @endif
    @endif
</div>
